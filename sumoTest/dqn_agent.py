# dqn_agent.py - ИСПРАВЛЕННАЯ ВЕРСИЯ
import torch
...
class DQN_LSTM(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_size=64, num_layers=1):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.2),  # Добавим dropout для регуляризации
            nn.Linear(128, 128),
            nn.ReLU(),
...
class DQNAgent:
    def __init__(self, state_size, action_size, sequence_length=5, lr=0.0005, gamma=0.99,
        ...
        self.q_network = DQN_LSTM(state_size, action_size).to(self.device)
        self.target_network = DQN_LSTM(state_size, action_size).to(self.device)
        self.optimizer = optim.Adam(self.q_network.parameters(), lr=lr, weight_decay=1e-5)
        self.update_target()
        
        # Текущая последовательность состояний
    ...
    def remember_sequence(self, state, action, reward, next_state, done):
        """Сохраняем полную последовательность"""
        if len(self.current_sequence) < self.sequence_length:
            # Заполняем последовательность начальными состояниями
    ...
    def replay(self):
        ...
        # ИСПРАВЛЕННАЯ СТРОКА - правильные скобки
        states = np.array([item[0] for item in batch])
        actions = np.array([item[1] for item in batch])
        rewards = np.array([item[2] for item in batch])
        
        # Создаем next_states путем сдвига последовательности
        next_states_list = []
        for item in batch:
            current_seq = item[0]  # текущая последовательность
            next_state = item[3]   # следующее состояние
            # Сдвигаем последовательность: убираем первое состояние, добавляем следующее
    ...
    def reset_sequence(self):
        """Сброс последовательности при новом эпизоде"""
