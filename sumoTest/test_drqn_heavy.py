...
def main():
    ...
    sequence_length = 5
    env = SumoHistoryEnv(
        sumo_cfg="heavy_test.sumocfg",   # ← главное изменение!
        gui=False,                        # ← без GUI для точности и скорости
        max_steps=7200,                   # 2 часа
        sensor_failure_prob=0.05,         # реалистичная неопределённость
    ...
    try:
        ...
        # Загружаем обученную DRQN-модель
        ...
        while True:
            ...
            # Выполняем шаг с длительностью фазы = 10 сек (как при обучении!)
            ...
            if step_count % 20 == 0:  # каждые ~200 сек симуляции
                avg_wait_step = total_wait / (step_count * 10)
                print(f"⏱️ Шаг {step_count*10} сек | Фаза {action} | Ср. ожидание: {avg_wait_step:.2f} сек")

            if done:
                break

        # Финальные метрики
        sim_time_sec = step_count * 10  # каждый шаг = 10 сек
        avg_wait = total_wait / sim_time_sec if sim_time_sec > 0 else 0
        print("\n" + "="*60)
        print("📊 РЕЗУЛЬТАТЫ DRQN НА HEAVY-СЦЕНАРИИ:")
        print(f"⏱️ Продолжительность симуляции: {sim_time_sec} сек ({sim_time_sec/3600:.1f} ч)")
